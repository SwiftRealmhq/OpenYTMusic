...
import com.rootleo.velqi.db.entities.Album
import com.rootleo.velqi.db.entities.AlbumArtistMap
import com.rootleo.velqi.db.entities.AlbumEntity
import com.rootleo.velqi.db.entities.AlbumWithSongs
import com.rootleo.velqi.db.entities.Artist
import com.rootleo.velqi.db.entities.ArtistEntity
import com.rootleo.velqi.db.entities.Event
import com.rootleo.velqi.db.entities.EventWithSong
import com.rootleo.velqi.db.entities.FormatEntity
import com.rootleo.velqi.db.entities.LyricsEntity
import com.rootleo.velqi.db.entities.Playlist
import com.rootleo.velqi.db.entities.PlaylistEntity
import com.rootleo.velqi.db.entities.PlaylistSong
import com.rootleo.velqi.db.entities.PlaylistSongMap
...
import com.rootleo.velqi.db.entities.SongAlbumMap
import com.rootleo.velqi.db.entities.SongArtistMap
import com.rootleo.velqi.db.entities.SongEntity
import com.rootleo.velqi.db.entities.SongWithPlayCount
import com.rootleo.velqi.extensions.reversed
import com.rootleo.velqi.extensions.toSQLiteQuery
import com.rootleo.velqi.models.MediaMetadata
import com.rootleo.velqi.models.toMediaMetadata
import com.rootleo.velqi.ui.utils.resize
...
interface DatabaseDao {
    ...
    // ===== Estadisticas reales (StatsScreen) =====

    // Tiempo total escuchado en el periodo (suma de playTime de los eventos)
    @Query("SELECT COALESCE(SUM(playTime), 0) FROM event WHERE timestamp > :fromTimeStamp")
    fun totalPlayTime(fromTimeStamp: Long): Flow<Long>

    // Numero total de reproducciones en el periodo
    @Query("SELECT COUNT(1) FROM event WHERE timestamp > :fromTimeStamp")
    fun totalPlayCount(fromTimeStamp: Long): Flow<Int>

    // Canciones con su numero de veces reproducidas en el periodo
    @Query(
        """
        SELECT song.*, COUNT(event.playTime) AS playCount
        FROM song
                 JOIN event ON event.songId = song.id
        WHERE event.timestamp > :fromTimeStamp
        GROUP BY song.id
        ORDER BY playCount DESC, SUM(event.playTime) DESC
        LIMIT :limit
    """
    )
    fun songPlayCounts(fromTimeStamp: Long, limit: Int = 10): Flow<List<SongWithPlayCount>>

    // Eventos del periodo (para el desglose por dia en Kotlin)
    @Query("SELECT * FROM event WHERE timestamp > :fromTimeStamp ORDER BY timestamp")
    fun eventsSince(fromTimeStamp: Long): Flow<List<Event>>
